...
void jar_xm_generate_samples_8bit(jar_xm_context_t* ctx, char* output, size_t numsamples) {
    ...
    if(output){
        for(int x=0;x<2*numsamples;x++) output[x] = (char)(musicBuffer[x] * 127.0f); // scale sample to signed 8 bit
    }
    JARXM_FREE(musicBuffer);
}
        ...
        if(ch->sample != 0) { //&& NOTE_IS_VALID(s->note)) {
            ...
            case jar_xm_FORWARD_LOOP:
                if (final_offset >= ch->sample->loop_end) {
                    ...
                    ch->sample_position = (float)ch->sample->loop_start;
                } else {
                    ch->sample_position = (float)final_offset;
                }
                break;
...
static void jar_xm_tick(jar_xm_context_t* ctx) {
    ...
    for(uint8_t i = 0; i < ctx->module.num_channels; ++i) {
        ...
        panning = ch->panning + (ch->panning_envelope_panning - .5f) * (.5f - (float)fabs((double)(ch->panning - .5f))) * 2.0f;
        if(ch->tremor_on) {
            volume = .0f;
        } else {
...
//    ch->curr_right = 0.f;
    ...
    if(mod->linear_interpolation) {
        b = (uint32_t)ch->sample_position + 1;
...
const char* xm_octave_chr(int number) {
    ...
    int result = (int)(floor((double)number2 / 12.0) + 1.0);
    switch(result) {
    case 1: return "1";
    case 2: return "2";
    case 3: return "3";
